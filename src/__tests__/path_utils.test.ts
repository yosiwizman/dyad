...
describe("safeJoin", () => {
  ...
  describe("safe paths", () => {
    it("should join simple relative paths", () => {
      ...
      // safeJoin normalizes to forward slashes for cross-platform consistency
    ...
    it("should work with actual temp directory", () => {
      ...
      // safeJoin normalizes to forward slashes, so we normalize the expected path too
      const expected = tempDir.replace(/\\/g, "/") + "/test/file.txt";
  ...
  describe("edge cases", () => {
    it("should handle Windows-style base paths", () => {
      ...
      // safeJoin normalizes to forward slashes
      expect(result).toBe("C:/app/workspace/src/file.txt");
    });
